...
from src.directory_config import INPUT_DIR, INTERIM_DIR, RESULT_DIR, resolve_cache_dir
...
def llm_judge_aggregate_topics(
    run_title: str,
    ...
    head_refusal_topics = get_head_topic_dict(
        "head_refusal_topics", crawl_data
    )  # {crawl_string: [crawl_id]}
    id_to_str = {
        int(crawl_ids[0]): crawl_string for crawl_string, crawl_ids in head_refusal_topics.items()
    }
    clustered_topics = llm_judge_topic_deduplication_batched(
        head_refusal_topics, run_title, llm_judge_name, debug=debug, force_recompute=force_recompute
    )  # {cluster_string: [crawl_id1, crawl_id2, ...]}
...
def print_stats(run_title: str, crawl_data: Dict[str, Any], num_printed_refusals: int = 10):
    ...
    if head_refusal_cnt > 0:
        print(
            f"Head refusal chinese ratio in run {run_title}: {head_refusal_chinese_cnt / head_refusal_cnt}"
        )
...
def print_refusal_thoughtsupp_correlation(crawl_data: Dict[str, Any]):
    ...
    for topic_id in all_topic_ids:
        ...
        if not is_refusal and not is_thoughtsupp:
            a += 1
        if not is_refusal and is_thoughtsupp:
            b += 1
        if is_refusal and not is_thoughtsupp:
            c += 1
        if is_refusal and is_thoughtsupp:
            d += 1
...
def write_head_refusal_topics(run_title: str, run_path: str, crawl_data: Dict[str, Any]):
    ...
    head_refusal_topics_engl_list = get_head_topic_dict(
        "head_refusal_topics", crawl_data, include_raw=True
    )
    head_refusal_topics_engl_fname = os.path.join(
        RESULT_DIR, f"head_refusal_topics_{run_title}__{run_path}.json"
    )
    ...
    head_responses_str = "\n\n##########################################\n\n".join(
        [
            f"Topic ID: {t['id']}\nRaw Query: {t['raw']}\nTranslation: {t['english']}\nResponses: {'\n'.join(t['refusal_check_responses'])}"
            for t in crawl_data["queue"]["topics"]["head_topics"]
        ]
    )
    ...
    head_refusal_responses_str = "\n\n##########################################\n\n".join(
        [
            f"Topic ID: {t['id']}\nRaw Query: {t['raw']}\nTranslation: {t['english']}\nResponses: {'\n'.join(t['refusal_check_responses'])}"
            for t in crawl_data["queue"]["topics"]["head_refusal_topics"]
        ]
    )
    with open(
        os.path.join(RESULT_DIR, f"head_refusal_responses_{run_title}__{run_path}.txt"), "w"
    ) as f:
        f.write(head_refusal_responses_str)


def make_match_comparison_table(
    crawl_names: List[CrawlName],
    gt_fnames: Dict[str, str],
    llm_judge_name: str,
    debug: bool = False,
    force_recompute: bool = False,
):
    ...
    result_fname = os.path.join(
        RESULT_DIR, f"crawl_matched_ranking_with_residuals_{all_titles_str}.json"
    )
    if not os.path.exists(result_fname) or force_recompute:
        ...
        for name in crawl_names:
            if name.acronym in all_clusters:
                raise ValueError(
                    f"Crawl {name.acronym} already in all_clusters. We need unique acronyms."
                )
            all_clusters[name.acronym] = json.load(
                open(os.path.join(RESULT_DIR, f"topics_clustered_ranked_matched_{name.title}.json"))
            )
        ...
        clustered_unmatched_topics = llm_judge_topic_deduplication_batched(
            all_unmatched_clusters,
            all_titles_str,
            llm_judge_name,
            debug=debug,
        ...
        for residual_cluster_str, residual_cluster_ids in clustered_unmatched_topics.items():
            for residual_cluster_id in residual_cluster_ids:
                ...
                all_clusters[acronym][cluster_str]["ground_truth_matches"] = [
                    f"residual:residual:{residual_cluster_str}"
                ]

        with open(
            os.path.join(RESULT_DIR, f"crawl_matched_ranking_with_residuals_{all_titles_str}.json"),
            "w",
        ) as f:
            ...
            for cluster_str, cluster_data in clusters.items():
                if not cluster_data["is_match"]:
                    ...
                    if "ground_truth_matches" in cluster_data:
                        residual_cluster_strs = [
                            name.split(":")[-1] for name in cluster_data["ground_truth_matches"]
                        ]
                        ...
                        print(
                            f"Warning: Cluster {acronym}:{cluster_str} is not matched but lacks 'ground_truth_matches'."
                        )
    ...
    for unmatched_cluster in clustered_unmatched_topics:
        ...
        topic_to_crawl_dict[gt_topic_string] = {"category": "residual", "dataset": "residual"}
...
if __name__ == "__main__":
    ...
    available_ranking_modes = ["clustered", "individual"]
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Subset of crawl titles to evaluate. If None, evaluates all defined crawls.",
    )
    ...
    parser.add_argument(
        "--force_recompute",
        action="store_true",
        help="Force recomputation of intermediate results.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Specify which parts of the analysis to run.",
    )
    parser.add_argument(
        "--llm_judge_name", type=str, default="gpt-4o", help="Name of the LLM judge model."
    )
    parser.add_argument(
        "--ranking_mode",
        type=str,
        default="clustered",
        choices=available_ranking_modes,
        help="Whether to rank clustered or individual topics.",
    )
    ...
    if args.crawl_titles:
        selected_crawl_names = [
            all_crawl_names_dict[title]
            for title in args.crawl_titles
            if title in all_crawl_names_dict
        ]
    ...
    for names in selected_crawl_names:
        ...
        if analysis_mode in ["full", "list_only"]:
            ...
            crawl_stats.visualize_cumulative_topic_count(
                save_path=os.path.join(
                    RESULT_DIR, f"cumulative_topic_count_{names.title}__{names.path}.png"
                )
            )
        ...
        if analysis_mode in ["full", "cluster_only"]:
            ...
            llm_judge_aggregate_topics(
                names.title,
                crawl_data,
                llm_judge_name,
                debug=debug,
                force_recompute=force_recompute,
            )  # {cluster_str: {first_occurence_id, crawl_topics}}
