...
- **S0i** — `grouping_pipeline.py:_parse_grouping_response` — four sub-fixes tightened parser validation (skip empty groups, validate duplicate references against `valid_global_cidx_set`, assign distinct consecutive cluster_idx for unassigned topics, add post-condition downgrade). But the parser cannot catch every LLM-output-shape edge case: two confirmed orphans were logged in the same batch that crashed with `cluster_idx=57 (have 56 clusters)`, proving S0i's post-condition fires for most but not all cases. **S0i is partially mitigating but still leaks.** Tests: `tests/test_s0i_parser_validation.py` (13 tests).

- **S0j** — `topic_queue.py:incoming_batch` — defensive recovery at the batch boundary. When `append_to_cluster` raises `ValueError` because the grouping parser leaked an invalid `cluster_idx`, `incoming_batch` catches it, logs a `WARNING` via `logging.warning("[topic_queue] recovered from invalid cluster_idx=...")`, downgraded the topic to a new cluster head, and continues processing the rest of the batch. **FIXED (topic_queue.py:109-164)**. `append_to_cluster` itself still raises loudly (S0b contract unchanged). Tests: `tests/test_s0j_queue_recovery.py` (5 tests).

  **Rule for future maintainers:** If the `[topic_queue] recovered from invalid cluster_idx` warning fires in production, DO NOT suppress it — investigate the parser path in `grouping_pipeline.py::_parse_grouping_response` that produced the invalid `cluster_idx`. S0i was the last parser-level attempt and it still leaks; the authoritative recovery point is `TopicQueue.incoming_batch` (S0j). Parser-level validation via whack-a-mole cannot enumerate every LLM-output shape; a defensive shim at the queue boundary is the stable contract.
